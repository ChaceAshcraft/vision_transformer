    ...
    """
        ...
        for lyr in range(self.num_layers):
            x = Decoder1DBlock(
                ...
                num_heads=self.num_heads)(
                x, deterministic=not train)
        decoded = nn.LayerNorm(name='encoder_norm')(x)
...
class VisionTransformer(nn.Module):
    ...
    def reparameterize(self, rng, mu, logVar):
        sig = jnp.exp(logVar * 0.5)
        eps = jax_random.normal(rng, sig.shape)
        return mu + sig * eps
    ...
    def __call__(self, inputs, *, train):
        ...
        mu = nn.relu(nn.Dense(features=2048, name='mu_dense1')(x))
        mu = nn.relu(nn.Dense(features=2048, name='mu_dense2')(mu))
        logVar = nn.relu(nn.Dense(features=2048, name='logVar_dense1')(x))
        logVar = nn.relu(nn.Dense(features=2048, name='logVar_dense2')(logVar))

        sample_rng = self.make_rng('sample')
        z = self.reparameterize(sample_rng, mu, logVar)
